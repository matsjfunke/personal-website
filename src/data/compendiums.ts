import { CompendiumMeta } from "@/types/compendium";

export const compendiums: CompendiumMeta[] = [
  {
    title: "OAuth 2.0 Authorization Flow",
    description:
      "Understanding the OAuth 2.0 authorization flow for secure third-party access",
    date: "2025-03-06",
    author: "matsjfunke",
    slug: "oauth2",
  },
  {
    title: "Basic Unix File and Directory Permissions",
    description: "Overview, explanation and how to change them",
    date: "2025-01-19",
    author: "elfeldos & matsjfunke",
    slug: "file-permissions",
  },
  {
    title: "Transformer / Encoder-Decoder Architecture",
    description:
      "Notes on the Transformer architecture, including tokenization, embedding, positional encoding, encoder, decoder, and training.",
    date: "2025-01-13",
    author: "matsjfunke",
    slug: "transformer",
  },
  {
    title: "Rust Book",
    description: "Condensed summary of key rust concepts from rust book",
    date: "2025-01-12",
    author: "matsjfunke",
    slug: "rust",
  },
  {
    title: "Thoughts on Tensors in DL",
    description:
      "Tensor concepts for deep learning: how DL tensors differ from mathematical tensors, core concepts, manipulation functions, and memory implementation.",
    date: "2025-01-10",
    author: "matsjfunke",
    slug: "tensors",
  },
  {
    title: "How to configure firewalls for VPS security?",
    description: "Guide to Setting Up Firewalls on Your Linux VPS",
    date: "2024-12-25",
    author: "matsjfunke",
    slug: "firewalls",
  },
  {
    title: "The Rsync Guide",
    description: "Everything you should know about Rsync",
    date: "2024-12-21",
    author: "matsjfunke",
    slug: "rsync",
  },
  {
    title: "The SSH Guide",
    description: "Everything you should now about SSH",
    date: "2024-12-21",
    author: "matsjfunke",
    slug: "ssh",
  },
  {
    title: "Hetzner VPS Setup",
    description: "Getting Started Tutorial for Hetzner VPS",
    date: "2024-12-20",
    author: "matsjfunke",
    slug: "hetzner",
  },
  {
    title: "DigitalOcean Droplet Setup",
    description: "Getting Started Tutorial for DigitalOcean Droplet / VPS",
    date: "2024-10-22",
    author: "matsjfunke",
    slug: "digital-ocean",
  },
  {
    title: "Setting up DNS for Your Domain",
    description: "A comprehensive guide to configuring DNS for your website",
    date: "2024-03-15",
    author: "matsjfunke",
    slug: "dns-setup",
  },
  {
    title: "Docker & Docker Compose on VPS",
    description: "Tutorial to install Docker & Docker Compose on a Linux VPS",
    date: "2024-03-10",
    author: "matsjfunke",
    slug: "linux-docker",
  },
];
